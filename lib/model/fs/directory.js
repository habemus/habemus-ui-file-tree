...
function HFSDir(data) {
  ...
  if (this.isRoot) {
    ...
    // rootDir directory listens to hfs events
    rootDir.hfs.subscribe('file-removed', function (data) {
      rootDir.ensureDoesNotExist('leaf', data.path);
    });

    rootDir.hfs.subscribe('directory-removed', function (data) {
      rootDir.ensureDoesNotExist('branch', data.path);
    });

    rootDir.hfs.subscribe('file-created', function (data) {
      rootDir.ensureExists('leaf', data.path);
    });

    rootDir.hfs.subscribe('directory-created', function (data) {
...
HFSDir.prototype.fsRead = function () {
  ...
  return this.root.hfs.readDirectory(dirPath)
    .then(function (contents) {
      ...
      self.root.hfs.startWatching(dirPath);
