...
module.exports = function (happinessTree, rootModel, uiCore, options) {
  ...
  var LEAF_MENU_OPTIONS = [
    {
      group: 'self',
      label: _t('habemus-ui-file-tree.file.rename'),
        ...
        editableLabel.edit(editOptions, function (value) {
          if (value === '') {
            uiNotifications.error.show({
              text: _t('habemus-ui-file-tree.file.rename-error.empty-name'),
              duration: 3000,
            });
          ...
          uiNotifications.loading.show({
            text: _t('habemus-ui-file-tree.file.rename-loading'),
          ...
          return nodeModel.fsRename(value)
            .then(function (res) {
              ...
              uiNotifications.success.show({
                text: _t('habemus-ui-file-tree.file.rename-success'),
                duration: 3000,
              });
            ...
            .catch(function (err) {
              ...
              switch (err.name) {
                case 'PathExists':
                  uiDialogs.alert(
                    _t('habemus-ui-file-tree.file.rename-error.path-exists')
                  );
                  break;
                default:
                  uiNotifications.error.show({
                    text: _t('habemus-ui-file-tree.file.rename-error.general'),
                    duration: 3000,
                  });
                  break;
              }
    ...
    {
      group: 'auxiliary',
      label: _t('habemus-ui-file-tree.file.duplicate'),
      callback: function (data) {
        data.menuElement.close();
        var nodeModel = data.context;
        ...
        return Bluebird.all([
          uiDialogs.prompt(
            _t('habemus-ui-file-tree.file.duplicate-prompt'),
            {
              submit: _t('habemus-ui-file-tree.file.duplicate'),
        ...
        .then(function (results) {
          ...
          uiNotifications.loading.show({
            text: _t('habemus-ui-file-tree.file.duplicate-loading'),
            duration: Math.Infinity,
          });
        ...
        .then(function (res) {
          uiNotifications.loading.hide();
          uiNotifications.success.show({
            text: _t('habemus-ui-file-tree.file.duplicate-success'),
            duration: 3000,
          });
        ...
        .catch(function (err) {
          ...
          uiNotifications.loading.hide();
          uiNotifications.error.show({
            text: _t('habemus-ui-file-tree.file.duplicate-error.general'),
            duration: 3000,
          });
          console.warn(err);
        });
    ...
    },
    {
      ...
      callback: function (data) {
        ...
        return happinessTree.promptNewFile(nodeModel.parent);
      }
    },
    {
      group: 'new',
      ...
      callback: function (data) {
        ...
        return happinessTree.promptNewDirectory(nodeModel.parent);
      }
    },
    {
      group: 'self',
      label: _t('habemus-ui-file-tree.file.remove'),
      callback: function (data) {
        data.menuElement.close();
        var nodeModel = data.context;
        ...
        var msg = _t('habemus-ui-file-tree.file.remove-confirm', {
          path: path,
        });
        ...
        uiDialogs.confirm(msg)
          .then(function confrmed() {
            ...
            uiNotifications.loading.show({
              text: _t('habemus-ui-file-tree.file.remove-loading'),
              duration: Math.Infinity
            });
          ...
          .then(function (res) {
            uiNotifications.loading.hide();
            uiNotifications.success.show({
              text: _t('habemus-ui-file-tree.file.remove-success'),
              duration: 3000,
            });
          ...
          .catch(function (err) {
            ...
            uiNotifications.loading.hide();
            uiNotifications.error.show({
              text: _t('habemus-ui-file-tree.file.remove-error.general'),
              duration: 3000,
            });
            console.warn(err);
          });
    ...
    {
      group: 'auxiliary',
      label: _t('habemus-ui-file-tree.file.copy-path'),
      callback: function (data) {
        data.menuElement.close();
        var nodeModel = data.context;
        ...
        clipboard.copy(nodeModel.path)
          .then(function () {
            uiNotifications.success.show({
              text: _t('habemus-ui-file-tree.file.copy-path-success', {
                path: nodeModel.path,
              }),
              duration: 3000,
            });
    ...
    {
      group: 'auxiliary',
      label: _t('habemus-ui-file-tree.file.open-in-new-tab'),
      type: 'url',
      target: '_blank',
      url: function (data) {
        var nodeModel = data.context;
